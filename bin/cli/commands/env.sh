...
if [[ "${APP_ENV}" == "local" ]]; then
    ...
    prompt POSTGRES_EXTERNAL_PORT "Postgres external port" "${POSTGRES_EXTERNAL_PORT:-5432}"
    prompt VALKEY_EXTERNAL_PORT "Valkey external port" "${VALKEY_EXTERNAL_PORT:-6380}"
fi
...
VALKEY_EXTERNAL_PORT=${VALKEY_EXTERNAL_PORT:-6380}
ENVEOF
fi
